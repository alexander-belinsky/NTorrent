 ...
 *
 * @code yield m_socket->async_read_some(buffer(*buffer_), *this); @endcode
 *
 ...
 *   m_socket->async_read_some(b, *this);
 * } @endcode
 *
 ...
 *   do
 *   {
 *     m_socket.reset(new tcp::socket(my_context_));
 *     yield acceptor->async_accept(*m_socket, *this);
 *     fork server(*this)();
 *   } while (is_parent());
